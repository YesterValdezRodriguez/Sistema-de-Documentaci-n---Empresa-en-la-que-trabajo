...
(function () {
  ...
  const Icons = {
    ...
    grip: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="12" r="1"></circle><circle cx="9" cy="5" r="1"></circle><circle cx="9" cy="19" r="1"></circle><circle cx="15" cy="12" r="1"></circle><circle cx="15" cy="5" r="1"></circle><circle cx="15" cy="19" r="1"></circle></svg>',
    ...
    insert: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 10 4 15 9 20"></polyline><path d="M20 4v7a4 4 0 0 1-4 4H4"></path></svg>',
    ...
    arrowLeft: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>'
  };
  ...
  function toast(msg, tipo) {
    ...
    t.innerHTML = '<div class="toast-icon">' + (tipo === 'pel' ? Icons.info : Icons.check) + '</div><div>' + esc(msg) + '</div>';
    cont.appendChild(t);
    setTimeout(function () { t.classList.add('mostrar'); }, 10);
    setTimeout(function () { t.classList.remove('mostrar'); setTimeout(function () { t.remove(); }, 400); }, 4200);
  }
  ...
  function modal(titulo, cuerpoHTML, botones, opciones) {
    ...
    dlg.innerHTML =
      ...
      '<button class="x" type="button" aria-label="Cerrar">' + Icons.x + '</button></div>' +
  ...
  async function renderPlantillas() {
    ...
    let filas = plantillas.map(function (p) {
      return '<tr>' +
        ...
        '<td class="der">' +
          '<a class="btn mini prim" href="#/plantilla/' + p.id + '" title="Abrir / armar">' + Icons.edit + '</a> ' +
          '<button class="btn mini ok" data-generar="' + p.id + '" title="Generar expediente">' + Icons.play + '</button> ' +
    ...
    app.innerHTML =
      ...
      '<button class="btn prim" id="btnNueva">' + Icons.plus + ' Nueva plantilla</button></div>' +
  ...
  async function renderBuilder(id) {
    ...
    const itemsHTML = plantilla.items.map(function (it, idx) {
      ...
      const insHTML = inserciones.map(function (ins) {
        ...
        return '<div class="insercion">' +
          '<span class="flecha">' + Icons.insert + '</span>' +
          ...
          ' (' + paginasItem(ins, mapa) + ')' + (ins.paso ? (' · ' + esc(ins.paso) + (ins.sellarPaso ? ' ' + Icons.tag : '')) : '') + '</span></div>' +
          '<button class="btn mini prim" data-edit-ins="' + ins.id + '" data-item="' + it.id + '" title="Editar inserción">' + Icons.edit + '</button>' +
          '<button class="btn mini pel" data-quita-ins="' + ins.id + '" data-item="' + it.id + '" title="Quitar inserción">' + Icons.x + '</button>' +
          '</div>';
      }).join('');
      const insBloque = inserciones.length ? ('<div class="inserciones">' + insHTML + '</div>') : '';
      return '<div class="item" draggable="true" data-id="' + it.id + '">' +
        '<div class="item-fila">' +
          '<span class="grip" title="Arrastrar">' + Icons.grip + '</span>' +
          ...
          '<div class="muted small">' +
            ...
            (it.paso ? ('paso: ' + esc(it.paso) + (it.sellarPaso ? ' ' + Icons.tag : '')) : '<i>sin paso</i>') +
          '</div>' + (it.titulo ? '<div class="small">' + esc(it.titulo) + '</div>' : '') + '</div>' +
          '<div class="item-btns">' +
            '<button class="btn mini" data-grafico="' + it.id + '" title="Insertar gráficamente (ver páginas)">' + Icons.layout + '</button>' +
            '<button class="btn mini" data-ins="' + it.id + '" title="Insertar PDF entre páginas">' + Icons.insert + '</button>' +
            '<button class="btn mini" data-sube="' + it.id + '" ' + (idx === 0 ? 'disabled' : '') + ' title="Subir">' + Icons.up + '</button>' +
            '<button class="btn mini" data-baja="' + it.id + '" ' + (idx === plantilla.items.length - 1 ? 'disabled' : '') + ' title="Bajar">' + Icons.down + '</button>' +
            '<button class="btn mini prim" data-edit="' + it.id + '" title="Editar">' + Icons.edit + '</button>' +
    ...
    const formAgregar = lista.length ?
      ('<div class="card-foot"><div class="grid">' +
        ...
        '<div class="c12"><button class="btn prim" id="btnAgregar">' + Icons.plus + ' Agregar registro</button></div>' +
        '</div><div class="small muted" style="margin-top: 12px">Páginas: vacío = todas. Ej. <code>1-3</code>, <code>2,5</code>, <code>4-</code>. ' +
    ...
    app.innerHTML =
      '<div class="barra"><div><a href="#/plantillas" class="enlace small" style="display:inline-flex;align-items:center;gap:4px;margin-bottom:8px;">' + Icons.arrowLeft + ' Plantillas</a>' +
      ...
      '<div class="acciones">' +
        '<button class="btn" id="btnPreview">' + Icons.eye + ' Vista previa</button>' +
        '<button class="btn ok" id="btnGenerar">' + Icons.play + ' Generar expediente</button>' +
        '<button class="btn sec" id="btnDuplicar">' + Icons.copy + ' Guardar como nueva config.</button>' +
      '</div></div>' +
      '<div class="cols">' +
  ...
  async function dialogoInsercion(plantillaId, itemId, insId, onDone) {
    ...
    const cuerpo =
      '<p class="small muted" style="margin-bottom: 12px">Se intercala un PDF entre las páginas de <b>' + esc(baseA ? baseA.nombre : 'este documento') +
      ...
      '</div><div class="small muted" style="margin-top:12px">0 = al inicio del documento. Para que el PDF quede como página 4, ponga <b>3</b>. ' +
  ...
  async function dialogoEditarItem(plantillaId, itemId) {
    ...
    const cuerpo =
      '<p class="fuerte" style="margin-bottom:12px">' + esc(a ? a.nombre : '(archivo eliminado)') + '</p><div class="grid">' +
  ...
  function dialogoDuplicar(plantilla) {
    const cuerpo = '<p class="small muted" style="margin-bottom:12px">Crea una copia independiente (estructura y sellos). Útil para variantes de producto.</p>' +
  ...
  async function renderGrafico(plantillaId, itemId) {
    ...
    if (!pdfjsDisponible() || !vBase) {
      app.innerHTML = '<div class="card"><p>' +
        ...
        '</p><a class="btn" href="' + volver + '">' + Icons.arrowLeft + ' Volver</a></div>';
      return;
    }
    ...
    function slotHTML(pos) {
      ...
      const cards = ins.map(function (x) {
        ...
        return '<div class="ins-mini">' +
          ...
          '<div class="ins-mini-btns"><button class="btn mini prim" data-edit-ins="' + x.id + '" title="Editar">' + Icons.edit + '</button>' +
          '<button class="btn mini pel" data-quita-ins="' + x.id + '" title="Quitar">' + Icons.x + '</button></div></div>';
      }).join('');
      ...
      return '<div class="slot" data-pos="' + pos + '"><div class="slot-cap">' + etiqueta + '</div>' + cards +
        '<div class="slot-drop">+ Soltar aquí</div></div>';
    }
    ...
    app.innerHTML =
      '<div class="barra"><div><a href="' + volver + '" class="enlace small" style="display:inline-flex;align-items:center;gap:4px;margin-bottom:8px;">' + Icons.arrowLeft + ' Volver al armador</a>' +
      ...
      paleta + '<a class="btn" href="#/archivos" style="margin-top:10px; width:100%">' + Icons.plus + ' Cargar PDF</a></aside>' +
  ...
  function verPdf(titulo, bytes) {
    ...
    modal(titulo,
      ...
      '<div class="small muted" style="margin-top:12px">Si no se ve, <a class="enlace" href="' + url + '" target="_blank" rel="noopener">ábralo en una pestaña</a>.</div>',
  ...
  async function dialogoGenerar(plantillaId) {
    ...
    const cuerpo = '<div class="grid">' +
      ...
      '<div class="c12" style="margin-top:8px"><b>¿Qué desea hacer?</b></div>' +
  ...
  async function renderArchivos() {
    ...
    app.innerHTML =
      ...
      '<div class="barra"><h2>Registros / PDFs</h2><button class="btn prim" id="btnCargar">' + Icons.upload + ' Cargar PDF</button></div>' +
  ...
  async function renderArchivoDetalle(id) {
    ...
    const filas = versiones.map(function (v) {
      ...
      return '<tr' + (esActual ? ' class="fila-activa"' : '') + '>' +
        '<td class="fuerte" style="display:flex;align-items:center;gap:6px">v' + esc(v.numero) + (esActual ? '<span style="color:var(--success)">' + Icons.check + '</span>' : '') + '</td>' +
        ...
        '<td class="der"><button class="btn mini" data-verv="' + v.id + '" title="Ver">' + Icons.eye + '</button> ' +
          (esActual ? '' : '<button class="btn mini prim" data-actual="' + v.id + '" title="Marcar como actual">' + Icons.star + '</button>') +
        '</td></tr>';
    }).join('');

    app.innerHTML =
      '<div class="barra"><div><a href="#/archivos" class="enlace small" style="display:inline-flex;align-items:center;gap:4px;margin-bottom:8px;">' + Icons.arrowLeft + ' Registros / PDFs</a><h2>' + esc(a.nombre) + '</h2>' +
      ...
      '<button class="btn pel" id="btnDelArch">' + Icons.trash + ' Eliminar archivo</button></div>' +
      ...
      '<div class="col-der"><div class="card"><div class="card-head">Subir nueva versión</div><div class="grid">' +
        ...
        '<div class="c12"><button class="btn prim" id="btnNuevaVer">' + Icons.upload + ' Cargar y hacer actual</button></div>' +
        '</div></div></div></div>';
  ...
  async function renderArmados() {
    ...
    let filas = arr.map(function (e) {
      return '<tr>' +
        ...
        '<td class="der">' +
          '<button class="btn mini" data-ver="' + e.id + '" title="Ver">' + Icons.eye + '</button> ' +
          '<button class="btn mini pel" data-desc="' + e.id + '" title="Descargar">' + Icons.download + '</button> ' +
  ...
  async function renderDatos() {
    ...
    app.innerHTML =
      ...
      '<div class="acciones"><button class="btn prim" id="btnExport">' + Icons.download + ' Exportar respaldo (.json)</button>' +
      '<label class="btn sec" style="cursor:pointer; display:inline-flex; align-items:center; gap:6px;">' + Icons.upload + ' Importar respaldo<input type="file" id="impFile" accept="application/json,.json" hidden></label>' +
      '<button class="btn pel" id="btnBorrar">' + Icons.trash + ' Borrar todos los datos</button></div></div>';
